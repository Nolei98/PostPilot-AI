  ...
  async ({ event, step }) => {
    ...
    const post = await step.run("fetch-post", async () => {
      const { data } = await supabase
        .from("posts")
        .select(
          "id, client_id, news_item_id, format, hook, video_status, video_shape, render_token, template_applied, tpl_keyword, tpl_top_text, tpl_bottom_text, tpl_cta_enabled, tpl_color_background, tpl_color_accent, tpl_color_text, tpl_color_keyword_box"
        )
        .eq("id", postId)
        .maybeSingle();
      return data;
